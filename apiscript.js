    ...
    data.map((values)=>{
      ...
      <div class="card">
        <h5 class="card-header">${values.name.common}</h5>
          ...
          <p class="card-body">Area:${values.area}</p><br>
          <p class="card-body"><button id="btn" onclick="getpopupdata('${n}')" class="btn">Weather</button></p>
        </div>
      </td>
     </tr>
      </table>`;
...
function getpopupdata(values){
   ...
   const api="https://api.openweathermap.org/data/2.5/weather?q="+values+"&appid=66a3de0e3a42ed5daa4cbd69cd9ff105";
